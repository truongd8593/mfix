...
- On the file menu click on the ![new](../mfixgui/icons/newfolder.png) `New`
button
...
> Note: Here we could have entered numerical values for the coordinates, but it
...
- Select the already populated "Background IC" from the region list. This will
initialize the entire flow field with air.
...
bottom half of the domain with glass beads.
...
- click the ![new](../mfixgui/icons/all_region.png) button to create a new
region that covers the entire domain to be used for the bed initial condition.
  - Enter a name for the region in the `Name` field ("bed")
  - Change the `To Y` field to be "ymax/2"
- Click the ![new](../mfixgui/icons/bottom_region.png) button to create a new
region to be used by the gas inlet boundary condition.
  - Enter a name for the region in the `Name` field ("inlet")
- Click the ![new](../mfixgui/icons/top_region.png) button to create a new
...
> Note: By default, boundaries that are left undefined (here the left, right,
> front, and back planes) will behave as No-Slip walls.
...
- Save project by clicking ![save](../mfixgui/icons/save.png) button
- Run the project by clicking the ![play](../mfixgui/icons/play.png) button
- On the `Run` dialog, select the executable from the combo-box
- Click the `Run` button to actually start the simulation
