            ...
            case 7:
                {
                    ...
                    {
                        for (int i=0; i<NMAX[m+1]; ++i)  // +1 fix
                        {
...
void MfixData::GetVariableAtTimestep(int vari , int tstep , int kslice)
{
    ...
    nBytesSkip += (FILE_POSITION)512; // record with time in it
    ...
    if (kslice == -1)
    {
     ...
     //  IN_BIN_512R (in,&var[0],imax2*jmax2);
    }
    else
    {
       nBytesSkip += (FILE_POSITION)sizeof(float) * (FILE_POSITION)(imax2*jmax2) * (FILE_POSITION)kslice;
       in.seekg(nBytesSkip,ios::beg);
