  ...
  vtkInformationVector *outputVector)
{
  ...
  Grid.resize(CellZones.size());

  for(int test=0; test < (int)CellZones.size(); test++)
    Grid[test] = vtkUnstructuredGrid::New();

  for (int i = 0; i < (int)Cells.size(); i++)
    {
    int location = distance(CellZones.begin(),find(CellZones.begin(),
                            CellZones.end(), Cells[i].zone));
    if (Cells[i].type == 1 )
      {
      for (int j = 0; j < 3; j++)
        {
      ...
      Grid[location]->InsertNextCell(Triangle->GetCellType(),
      ...
      Grid[location]->InsertNextCell(Hexahedron->GetCellType(),
      ...
      Grid[location]->InsertNextCell(Pyramid->GetCellType(),
                                     Pyramid->GetPointIds());
      }
      ...
      Grid[location]->InsertNextCell(Wedge->GetCellType(),
                                     Wedge->GetPointIds());
      }
    else if (Cells[i].type == 7 )
      {
      ConvexPointSet->GetPointIds()->SetNumberOfIds(Cells[i].nodes.size());
      for (int j = 0; j < (int)Cells[i].nodes.size(); j++)
        {
        ConvexPointSet->GetPointIds()->SetId( j, Cells[i].nodes[j]);
        }
      Grid[location]->InsertNextCell(ConvexPointSet->GetCellType(),
                                     ConvexPointSet->GetPointIds());
      }
    }
  Cells.clear();

  //Scalar Data
  for (int l = 0; l < (int)ScalarDataChunks.size(); l++)
    {
    int location = distance(CellZones.begin(),
                            find(CellZones.begin(),
                                 CellZones.end(),
                                 ScalarDataChunks[l].zoneId));
    vtkDoubleArray *v = vtkDoubleArray::New();
    for (int m = 0; m < (int)ScalarDataChunks[l].scalarData.size(); m++)
      {
  ...
  ScalarDataChunks.clear();

  //Vector Data
  for (int l = 0; l < (int)VectorDataChunks.size(); l++)
    {
    int location = distance(CellZones.begin(),
                            find(CellZones.begin(),
                                 CellZones.end(),
                                 VectorDataChunks[l].zoneId));
    vtkDoubleArray *v = vtkDoubleArray::New();
    v->SetNumberOfComponents(3);
    for (int m = 0; m < (int)VectorDataChunks[l].iComponentData.size(); m++)
      {
  ...
  VectorDataChunks.clear();

  for(int addTo = 0; addTo < (int)CellZones.size(); addTo++)
    {
  ...
  vtkInformationVector *vtkNotUsed(outputVector))
{
  ...
  for (int i = 0; i < (int)SubSectionIds.size(); i++)
    {
    if (SubSectionSize[i] == 1)
      {
      this->CellDataArraySelection->
             AddArray(VariableNames[SubSectionIds[i]].c_str());
...
{
  this->FluentCaseFile.open(filename, ios::binary);
...
void vtkFLUENTReader::GetNumberOfCellZones()
{
  ...
  for (int i = 0; i < (int)Cells.size(); i++)
    ...
    if (CellZones.size() == 0)
      ...
      {
      match = 0;
      for (int j = 0; j < (int)CellZones.size(); j++)
        {
  ...
  while (GetCaseChunk())
    ...
    switch (index)
      ...
      case 2012:
        ...
        break;
      case 3012:
        GetCellsBinary();  // Should be the same as single precision..
                           //only grabbing ints.
        break;
      case 2013:
        GetFacesBinary();
...
vtkstd::vector<int> t;
  ...
  for (int i = 0; i < (int)Cells.size(); i++)
    ...
    if ( ((Cells[i].type == 1)  && (Cells[i].faces.size() != 3)) ||
      ...
      // Copy faces
      t.clear();
      for (int j = 0; j < (int)Cells[i].faces.size(); j++)
        {
...
void vtkFLUENTReader::PopulateWedgeCell(int i)
{
  ...
  for (int j = 0; j < (int)Cells[i].faces.size(); j++)
    {
    if ((Faces[Cells[i].faces[j]].type == 3) && (second == 0) &&
         (Cells[i].faces[j] != base))
      {
      top = Cells[i].faces[j];
      second = 1;
      }
...
void vtkFLUENTReader::PopulatePolyhedronCell(int i)
{
  ...
  for (int j = 0; j < (int)Cells[i].faces.size(); j++)
    ...
    for (int k = 0; k < (int)Faces[Cells[i].faces[j]].nodes.size(); k++)
      {
      int flag;
      flag = 0;
      // Is the node already in the cell?
      for (int n = 0; n < (int)Cells[i].nodes.size(); n++)
        {
...
void vtkFLUENTReader::GetData(int dataType)
{
  ...
  // Is this a cell zone?
  int zmatch = 0;
  for (int i = 0; i < (int)CellZones.size(); i++)
    {
  ...
  if (zmatch)
    ...
    if (size == 1)
      ...
      for (int i=firstId; i<=lastId; i++)
        ...
        VectorDataChunks[VectorDataChunks.size()-1].iComponentData.
                                                    push_back(tempx);
        VectorDataChunks[VectorDataChunks.size()-1].jComponentData.
                                                    push_back(tempy);
        VectorDataChunks[VectorDataChunks.size()-1].kComponentData.
                                                    push_back(tempz);
        }
      }
    else
