 ...
 *              12/12 Replace memcpy with code to use memmove, memcpy,
 *                      or do nothing depending on overlap (Eschenberg)
   $Source$
   $Author$
   $Revision$
   $Date$
...
InitBIO ()
{
  ...
  if (!bio_initialized) {
    /* Check for some pathological compiler characteristics */
    if (!((sizeof(char) < sizeof(int))
	  ...
	  && (sizeof(double) == 8))) {
      fprintf(stderr,
	      "libmri internal error: this compiler uses unsupported lengths for some variable types!\n");
      exit(-1);
    }
	      ...
	      long long n)
{
   ...
   source = (unsigned char *)a;

   if( buf != source ) {
      if( ( source+n <= buf ) || ( buf+n <= source ) )
	 memcpy( buf, a, n );
      else
         memmove( buf, a, n );
   }
  ...
  if (sizeof(short) == 2)
    {
      ...
      if( buf != source ) {
	 if( ( source+2*n <= buf ) || ( buf+2*n <= source ) )
	    memcpy( buf, a, 2*n );
	 else
	    memmove( buf, a, 2*n );
      }
  ...
  if (sizeof(int) == 4)
    {
      ...
      if( buf != source ) {
	 if( ( source+4*n <= buf ) || ( buf+4*n <= source ) )
	    memcpy( buf, a, 4*n );
	 else
	    memmove( buf, a, 4*n );
      }
...
void
BRdInt64Array (unsigned char *buf,
	       long long *a,
	       long long n)
{
  long long i, len;
...
void
BWrInt64Array (unsigned char *buf,
	       long long *a,
	       long long n)
{
  long long i, len;
  ...
  if (sizeof(long long) == 8)
    {
      ...
      if( buf != source ) {
	 if( ( source+8*n <= buf ) || ( buf+8*n <= source ) )
	    memcpy( buf, a, 8*n );
	 else
	    memmove( buf, a, 8*n );
      }
