...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
       ...
       DOUBLE PRECISION, PARAMETER :: mu_usr=0.001d0, dpdx_usr=1200.0d0, h_usr=0.01d0, L_usr=0.2d0
       ...
       DO J = 2, JMAX+1
          ...
          P_g_exact_c(i,j) = P0_usr + dpdx_usr*( (X_temp(IMAX+2,j) + X_temp(IMAX+1,j))*HALF - xctmp )   
          U_g_exact_c(i,j) = ONE/(TWO*mu_usr)*dpdx_usr*yctmp*(h_usr-yctmp)   
          V_g_exact_c(i,j) = ZERO
       END DO
       END DO
...
!!       CLOSE(780)
       ...
       DO J = 2, JMAX+2
          ...
          xtmp = X_temp(i,j)
          ytmp = Y_temp(i,j)
          P_g_exact_n(i,j) = P0_usr + dpdx_usr*( (X_temp(IMAX+2,j) + X_temp(IMAX+1,j))*HALF - xtmp )  
          U_g_exact_n(i,j) = 1.0/(2.0*mu_usr)*dpdx_usr*ytmp*(h_usr-ytmp)   
          V_g_exact_n(i,j) = 0.0
       END DO
       END DO
       ...
       DO I = 2, IMAX+2
       DO J = 2, JMAX+1
          yctmp = HALF*(Y_temp(i,j) + Y_temp(i,j+1))
          U_g_temp = ONE/(TWO*mu_usr)*dpdx_usr*yctmp*(h_usr-yctmp)   
