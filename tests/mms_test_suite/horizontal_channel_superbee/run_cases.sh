...
mpirun -np 4 $CASE_DIR/src/mfix.exe imax=32 jmax=32 nodesi=2 nodesj=2 nodesk=1 > out.log
cat $CASE_DIR/de_norms.dat >> $CASE_DIR/de_norms_collected.dat
rm $CASE_DIR/{HCS.*,de_norms.dat,out.log}

# Run mesh_64 (i.e., 64x64 for 2D, 64x64x64 for 3D)
echo "Running mesh_64..."
mpirun -np 4 $CASE_DIR/src/mfix.exe imax=64 jmax=64 nodesi=2 nodesj=2 nodesk=1 > out.log
cat $CASE_DIR/de_norms.dat >> $CASE_DIR/de_norms_collected.dat
rm $CASE_DIR/{HCS.*,de_norms.dat,out.log}

# Run mesh_128 (i.e., 128x128 for 2D, 128x128x128 for 3D)
echo "Running mesh_128..."
mpirun -np 4 $CASE_DIR/src/mfix.exe imax=128 jmax=128 nodesi=2 nodesj=2 nodesk=1 > out.log
cat $CASE_DIR/de_norms.dat >> $CASE_DIR/de_norms_collected.dat
rm $CASE_DIR/{HCS.*,de_norms.dat,out.log}
