...
implicit none
  ...
  write(21,*) 'variables="h""ug""vg""wg"'
  do i = 1,ndata
    write(21,200) 2**(ncases-i),(l2de(i,j),j=2,neq)
  end do
  close(21)

  open(22,file='de_linf.dat',status='unknown')
  write(22,*) 'variables="h""ug""vg""wg"'
  do i = 1,ndata
    write(22,200) 2**(ncases-i),(linfde(i,j),j=2,neq)
  end do
  close(22)

  open(23,file='ooa_l2.dat',status='unknown')
  write(23,*) 'variables="h""ug""vg""wg"'
  do i = 2,ndata
    do j = 1,5
      ooal2(j) = log((l2de(i-1,j)+fsmall)/(l2de(i,j)+fsmall))/log(2.0_dp)
    end do
    write(23,201) 2**(ncases-i),(ooal2(j),j=2,neq)
  end do
  close(23)

  open(24,file='ooa_linf.dat',status='unknown')
  write(24,*) 'variables="h""ug""vg""wg"'
  do i = 2,ndata
    do j = 1,5
      ooalinf(j) = log((linfde(i-1,j)+fsmall)/(linfde(i,j)+fsmall))/log(2.0_dp)
    end do
    write(24,201) 2**(ncases-i),(ooalinf(j),j=2,neq)
  end do
  close(24)
