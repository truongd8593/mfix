...
! loezos: Shear Source terms from convective mom. flux
            IF (SHEAR) THEN
                ...
                Source_conv=A_M(IJK,N,m)*VSH_n + A_M(IJK,S,m)*VSH_s +&
                   ...
                   (A_M(IJK,N,m) + A_M(IJK,S,m) + A_M(IJK,W,m) + &
                    A_M(IJK,E,m))*VSH_p
             ELSE 
                Source_conv=0d0
             ENDIF
...
! Collect the terms
            ...
            B_M(IJK,M) = B_M(IJK,M) - (SDP + TAU_V_G(IJK) + &
...
!! FLAGMMS: begin
               IF(USE_MMS) THEN
                 IF (BC_PLANE(L) == 'W' .OR. BC_PLANE(L) == 'E' .OR. &
                     BC_PLANE(L) == 'B' .OR. BC_PLANE(L) == 'T') THEN  
! if the fluid cell is on the east/west/top/bottom of the pressure outflow/inflow 
! boundary then make sure the source terms are zero
                   I1 = BC_I_W(L) 
                   I2 = BC_I_E(L) 
                   J1 = BC_J_S(L) 
                   J2 = BC_J_N(L) 
                   ...
                   DO K = K1, K2 
                      ...
                      ENDDO 
                   ENDDO     
                 ENDIF ! end of BC_PLANE(L)
               ENDIF ! end of USE_MMS
!! FLAGMMS: end               
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE POINT_SOURCE_V_G(A_M, B_M, IER) 
