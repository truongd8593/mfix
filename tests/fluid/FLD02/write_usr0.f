...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      ...
      CALL WRITE_DAT_HEADER('POST_TG.dat','T_g')
      CALL WRITE_NRM_HEADER('POST_TG_NORMS.dat','T_g')
...
! file unit for heat transfer data
      ...
      WRITE(fUnit,"('#',10x,'Tol Residual:  ',es13.6)") TOL_RESID_T
