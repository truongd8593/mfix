...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      ...
      if(NSTEP > 1) then
         ...
         call CALC_NORMS(pL1, pL2, pLI, P_G, P_GO)
         ...
         maxL2 = max(pL2, uL2, vL2, kL2, eL2)

         if(maxL2 < 1.0e-10 .and. NSTEP > 1000) TSTOP = TIME
         if(myPE == 0) write(*,"(I8,5(2x,es11.4))") &
            NSTEP-1, pL2, uL2, vL2, kL2, eL2
