...
! This file isn't needed for this simulation
      IER = 0
      RETURN
...
!
! calculate heat of reactions only if energy eq. are solved
            IF(ENERGY_EQ) THEN
	              DO M = 0, MMAX-1
           	      DO L = M+1, MMAX
                     RxH_xfr(M, L) = zero 
               	     IF(R_tmp(M,L) .NE. UNDEFINED)THEN
! phase-M is generated from phase-L
                        IF(R_tmp(M,L) > ZERO) then
                           DO N = 1, NMAX(M)
                              RxH_xfr(M, L) =  RxH_xfr(M, L) + &
                                 R_tmp(M,L) * X_tmp(M,L, N) * &
			                              CALC_H(IJK, M, N)
                           END DO 
! phase-L is generated from phase-M
                        else
                           DO N = 1, NMAX(L)
                              RxH_xfr(M, L) =  RxH_xfr(M, L) + &
                                 R_tmp(M,L) * X_tmp(M,L, N) * &
                   		...
                   		ELSEIF(R_tmp(L,M) .NE. UNDEFINED)THEN
! phase-L is generated from phase-M
                   		   IF(R_tmp(L,M)> ZERO) then
                           DO N = 1, NMAX(L)
                              RxH_xfr(M,L) =  RxH_xfr(M,L) - R_tmp(L,M) * &
                              X_tmp(L,M,N) * CALC_H(IJK,L,N) 
                           END DO
! phase-M is generated from phase-L
             		         else
                           DO N = 1, NMAX(M)
                              RxH_xfr(M,L) =  RxH_xfr(M,L) - R_tmp(L,M) * &
                                 X_tmp(L,M, N) * CALC_H(IJK, M, N)
                           END DO 
                        endif
	                    ENDIF
	                 ENDDO 
...
!
               HOR_G(IJK) = zero
               DO N = 1, NMAX(0)
                  HOR_G(IJK) = HOR_G(IJK) + (R_gp(IJK, N) - &
                     RoX_gc(IJK, N) * X_g(IJK, N)) * CALC_H(IJK, 0, N)
               END DO 
               DO L = 1, MMAX
               ...
               DO M = 1, MMAX 
                  HOR_s(IJK, M) = zero
                  DO N = 1, NMAX(M)
                     HOR_s(IJK, M) = HOR_s(IJK, M) + (R_sp(IJK,M,N) - &
                        RoX_sc(IJK,M,N) * X_s(IJK,M,N)) * CALC_H(IJK,M,N)
                  END DO 
                  DO L = 0, MMAX
