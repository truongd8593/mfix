...
! following code commented out for circle-advection case           
!      PART_COUNT = 0
!      DO M = 1, MMAX
...
! following code added for circle-advection case      
      ALLOCATE(x_store(PARTICLES,DIMN))

      ln = 0
      DO I = 1, 64
         phi_local = (I-1)*2.0d0*PI/64
         ln = ln + 1
         ...
         DES_VEL_OLD(LN, 1) = (sin(PI*XM))**2 * sin(2.d0*PI*YM)*&
            cos(PI*time/T_per)
