...
! file unit for heat transfer data
      ...
      DOUBLE PRECISION, DIMENSION(PARTICLES) :: angles
...
! Open the files.
      FNAME1 = 'POST_OMEGA_'//angle_s//'.dat'
      ...
      IF (.NOT.F_EXISTS1) THEN
         ...
         WRITE(outfile,"(8X,'Time',9X,'Stage', &
            8X,'Pos',13X,'X_Pos_MFIX',6X,'Y_Pos_MFIX',6X,'REL ERR',6X,'calc_om2',1X,'exp_om2')")
      ELSE
...
! Write the results to a file.
      WRITE(outfile,"(3x,F15.8,5x,I1,5X,F15.8,7(3x,F15.8))") lTime, &
         lStage, lPos_Y,DES_POS_new(1,particle_index),DES_POS_new(2,particle_index),Pos_rErr,om2,om2s(particle_index)
      CLOSE(outfile)
   enddo
