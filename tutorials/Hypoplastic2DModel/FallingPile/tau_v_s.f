...
! 
!                      Shear stresses (Hypoplasticity) 
      DOUBLE PRECISION S12E, S12W
! 
!
! loezos
...
! 2D Hypoplasticity model
! integral(dS12/dx dVol) = S12_e * Ayz_e - S12_w * Ayz_w
               S12E = AVG_Y_S(AVG_X_S(scalar(IJK,3),scalar(IJKE,3),I),&
                              AVG_X_S(scalar(IJKN,3),scalar(IJKNE,3),I),J)
               S12W = AVG_Y_S(AVG_X_S(scalar(IJKW,3),scalar(IJK,3),IM),&
                              AVG_X_S(scalar(IJKNW,3),scalar(IJKN,3),IM),J)
	       
	       SSX = SSX + S12E*AYZ_V(IJK) - S12W*AYZ_V(IMJK) 
! end of Hypoplasticity
