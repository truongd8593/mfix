...
! Store values of D_s to be used for hypoplasticity	    
	    D_s11(IJK,M) = D_s(1,1)
            D_s12(IJK,M) = D_s(1,2)
            D_s22(IJK,M) = D_s(2,2)
! Store vorticity components
            vort_s12(IJK,M) = HALF * ( (U_s_N - U_s_S) * oDY(J) - &
            (V_s_E - V_s_W) * oDX(I) )  ! vort_s21 = -vort_s12
      
            IF(.NOT. NO_K) THEN
               D_s13(IJK,M) = D_s(1,3)
               D_s23(IJK,M) = D_s(2,3)
               D_s33(IJK,M) = D_s(3,3)
! Store vorticity components TO BE DONE LATER
            ENDIF
