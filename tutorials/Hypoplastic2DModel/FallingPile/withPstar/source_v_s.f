...
!
		     ...
		     IF(MMAX > 1) THEN ! extra work is done only in case of polydispersity.
		       ...
		       SDPS = - ((P_S(IJKN,M)-P_S(IJK,M))+(EPGA/SUM_EPS_CP)* &
                               (P_STAR(IJKN)-P_STAR(IJK))-(EPGA/SUM_EPS_CP)* &
                           (SCALAR(IJKN,2)- &
		           SCALAR(IJK,2&
                          )))*AXZ(IJK) 
		     ELSE
		         SDPS = - ((P_S(IJKN,M)-P_S(IJK,M))+(P_STAR(IJKN)-P_STAR(IJK))&
			 -(SCALAR(IJKN,2)-SCALAR(IJK,2)))*AXZ(IJK)
		     ENDIF
                  ELSE 
...
!             JEG Modified--University of Colorado, Hrenya Research Group
		        ...
		        +Source_conv+((V0+ZMAX((-VMT)))&
                        *V_SO(IJK,M)+VBF)*VOL_V(IJK))+B_M(IJK,M) 
!write(*,'(12(G15.6, 1X))') A_M(IJK,0,M), B_M(IJK,M), SDP, SDPS, TAU_V_S(IJK,M), Source_conv, V_SO(IJK,M), VBF
                  ENDIF
               ENDIF 
              END DO
!	      stop
