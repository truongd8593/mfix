...
!
       DO N = 1, NScalar ! Nscalar = 3 for 2D hypoplastic model
	    
	  M = Phase4Scalar(N)
	    
          CALL INIT_AB_M (A_M, B_M, IJKMAX2, M, IER) 
	    
	  IF(M == 1) THEN ! Stresses are transported with solids phase only, case M==0 was removed
...
!
	   ...
	   ! IF(N==3) THEN  ! uncomment to set component(s) of stress to zero
	   !  DO IJK = IJKSTART3, IJKEND3
	   !  A_M(ijk,-1,m) = ZERO
	   !  A_M(ijk,1,m) = ZERO
	   !  A_M(ijk,-2,m) = ZERO
