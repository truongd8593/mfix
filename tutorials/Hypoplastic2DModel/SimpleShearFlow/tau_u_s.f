...
! integral(dS12/dy dVol) = S12_n * Axz_n - S12_s * Axz_s
               S12N = AVG_X_S(AVG_Y_S(scalar(IJK,3),scalar(IJKN,3), J),&
                              AVG_Y_S(scalar(IJKE,3),scalar(IJKNE,3), J), I)
               S12S = AVG_X_S(AVG_Y_S(scalar(IJKS,3),scalar(IJK,3), JM),&
                              AVG_Y_S(scalar(IJKSE,3),scalar(IJKE,3), JM), I)
               
	       SSY = SSY + S12N*AXZ_U(IJK) - S12S*AXZ_U(IJMK) 
