...
!     defining parametrs for Simonin and Ahmadi models
      ...
      DOUBLE PRECISION f1, f2, df1, df2, d2f1, d2f2, d3f1, d3f2, d4f1, d4f2, d5f1, d5f2, d6f1, d6f2, d7f1, d7f2
      DOUBLE PRECISION shearStress, grad_s_avg(IMAX2)
!      
!                       term1 = f1 * f2; term2 = df1 * df2 * phi' th'; etc.
      DOUBLE PRECISION x1, term1(IMAX2), term2(IMAX2), term3(IMAX2), term4(IMAX2), &
      ...
      Double Precision term48(IMAX2),term49(IMAX2),term50(IMAX2),term51(IMAX2),term52(IMAX2),term53(IMAX2), &
			...
			term60(IMAX2),term61(IMAX2),term62(IMAX2), term63(IMAX2),term64(IMAX2)
!     
!     Sum of ep_s * g_0
...
! 
! Correlation for pressure for steady-state calculations.
	 x1 = ONE - Ep_g_mean(I)
	 f1  = x1 * (ONE + 4d0*Eta*G_0(IJK,M,M)*x1)
	 
	 df1 = x1*(4d0*Eta*(0.5d0*x1**2/(1d0-x1)**3+1.5d0*x1/(1d0-x1)**2+1d0/(1d0-x1))+4d0*Eta*x1*(1.5d0*x1**2 &
               /(1d0-x1)**4+4.0d0*x1/(1d0-x1)**3+2.5d0/(1d0-x1)**2))+4d0*Eta*x1*(0.5d0*x1**2/(1d0 &
              -x1)**3+1.5d0*x1/(1d0-x1)**2+1d0/(1d0-x1))+1d0
	 d2f1 = (x1*(8d0*Eta*(1.5d0*x1**2/(1d0-x1)**4+4.0d0*x1/(1d0-x1)**3+2.5d0/(1d0-x1)**2)+4d0*Eta*x1*( &
                6.0d0*x1**2/(1d0-x1)**5+15.0d0*x1/(1d0-x1)**4+9.0d0/(1d0-x1)**3))+8d0*Eta*(0.5d0*x1**2 &
               /(1d0-x1)**3+1.5d0*x1/(1d0-x1)**2+1d0/(1d0-x1))+8d0*Eta*x1*(1.5d0*x1**2/(1d0-x1)**4+4.0d0 &
               *x1/(1d0-x1)**3+2.5d0/(1d0-x1)**2))/2d0
	 d3f1 = (x1*(12d0*Eta*(6.0d0*x1**2/(1d0-x1)**5+15.0d0*x1/(1d0-x1)**4+9.0d0/(1d0-x1)**3)+4d0*Eta*x1 &
               *(30.0d0*x1**2/(1d0-x1)**6+72.0d0*x1/(1d0-x1)**5+42.0d0/(1d0-x1)**4))+24d0*Eta*(1.5d0 &
               *x1**2/(1d0-x1)**4+4.0d0*x1/(1d0-x1)**3+2.5d0/(1d0-x1)**2)+12d0*Eta*x1*(6.0d0*x1**2 &
              /(1d0-x1)**5+15.0d0*x1/(1d0-x1)**4+9.0d0/(1d0-x1)**3))/6d0
	 d4f1 = (x1*(16d0*Eta*(30.0d0*x1**2/(1d0-x1)**6+72.0d0*x1/(1d0-x1)**5+42.0d0/(1d0-x1)**4)+4d0*Eta &
                *x1*(180.0d0*x1**2/(1d0-x1)**7+420.0d0*x1/(1d0-x1)**6+240.0d0/(1d0-x1)**5))+48d0*Eta &
                *(6.0d0*x1**2/(1d0-x1)**5+15.0d0*x1/(1d0-x1)**4+9.0d0/(1d0-x1)**3)+16d0*Eta*x1*(30.0d0 &
                *x1**2/(1d0-x1)**6+72.0d0*x1/(1d0-x1)**5+42.0d0/(1d0-x1)**4))/24d0
	 d5f1 = (x1*(20d0*Eta*(180.0d0*x1**2/(1d0-x1)**7+420.0d0*x1/(1d0-x1)**6+240.0d0/(1d0-x1)**5)+4d0*  &
                Eta*x1*(1260.0d0*x1**2/(1d0-x1)**8+2880.0d0*x1/(1d0-x1)**7+1620.0d0/(1d0-x1)**6)) &
               +80d0*Eta*(30.0d0*x1**2/(1d0-x1)**6+72.0d0*x1/(1d0-x1)**5+42.0d0/(1d0-x1)**4)+20d0*Eta &
               *x1*(180.0d0*x1**2/(1d0-x1)**7+420.0d0*x1/(1d0-x1)**6+240.0d0/(1d0-x1)**5))/120d0
	 d6f1 = (x1*(24d0*Eta*(1260.0d0*x1**2/(1d0-x1)**8+2880.0d0*x1/(1d0-x1)**7+1620.0d0/(1d0-x1)**6) &
                +4d0*Eta*x1*(10080.0d0*x1**2/(1d0-x1)**9+22680.0d0*x1/(1d0-x1)**8+12600.0d0/(1d0-x1 &
                )**7))+120d0*Eta*(180.0d0*x1**2/(1d0-x1)**7+420.0d0*x1/(1d0-x1)**6+240.0d0/(1d0-x1 &
                )**5)+24d0*Eta*x1*(1260.0d0*x1**2/(1d0-x1)**8+2880.0d0*x1/(1d0-x1)**7+1620.0d0/( &
                1d0-x1)**6))/720d0
	 d7f1 = (x1*(28d0*Eta*(10080.0d0*x1**2/(1d0-x1)**9+22680.0d0*x1/(1d0-x1)**8+12600.0d0/(1d0-x1)**7 &
                )+4d0*Eta*x1*(90720.0d0*x1**2/(1d0-x1)**10+201600.0d0*x1/(1d0-x1)**9+110880.0d0 &
                /(1d0-x1)**8))+168d0*Eta*(1260.0d0*x1**2/(1d0-x1)**8+2880.0d0*x1/(1d0-x1)**7+1620.0d0 &
                /(1d0-x1)**6)+28d0*Eta*x1*(10080.0d0*x1**2/(1d0-x1)**9+22680.0d0*x1/(1d0-x1)**8 &
                +12600.0d0/(1d0-x1)**7))/5040d0
	 ...
	 term62(I) = term62(I) * eps6_Th7(I)
         ...
         P_s_v(IJK) = term1(I)+term2(I)+term3(I)+term4(I)+term5(I)+term6(I)+term7(I)+ &
...
! 	write(*,'(I3, 8(1X,E13.6))') I, P_s_v(IJK)
         P_s_v(IJK) = (P_s_v(IJK)+epsUxUx(I)+x1*UxUx(I)) * RO_S(M)! +epsUxUx(I)+x1*UxUx(I)

!write(*,*) I, P_s_v(IJK), RO_S(M)*(epsUxUx(I)+x1*UxUx(I))
...
!!!!!!!!!!!!!!!!!!!!!!!!! deleted mu_star Using Gidaspow Model !!!!!!!!!!!!
!                                               --------
!     shear viscosity in Mth solids phase  (add to frictional part)
            Mu_s_v(IJK) = Mu_s_v(IJK)/(ETA*G_0(IJK,M,M))*(ONE+1.6d0*ETA*G_0(IJK,M,M)*EP_s(IJK,M))**2 &
	                + (0.6d0*Mu_b_v(IJK)*Eta) ! Gidaspow viscosity
!     
!!!!!!!!!!!!!!!!!!!!!!!
          
	 x1 = ONE - Ep_g_mean(I)
	 grad_s_avg(I) = (V_s_mean(I+1) -V_s_mean(I-1) )/2d0 !( V_s(FUNIJK(I+1,J,K),M) - V_s(FUNIJK(I-1,J,K),M) )/2d0
!	 If(I == 42) grad_s_avg(I) = -grad_s_avg(I-1)
	 ...
	 term1(I) = term1(I)*grad_s_avg(I)*8d0
	 
	 term2(I) = term2(I) * eps_Th(I)*grad_s_avg(I)*8d0 + term2(I) * eps_Thg(I) * 8d0
	 
	 term3(I) = term3(I) * Th2(I)*grad_s_avg(I)*8d0 + term3(I) * Th2g(I) * 8d0
	 term4(I) = term4(I) * eps2(I)*grad_s_avg(I)*8d0 + term4(I) * eps2g(I) * 8d0
	 term5(I) = term5(I) * eps_Th2(I)*grad_s_avg(I)*8d0 + term5(I) * eps_Th2g(I) * 8d0
	 term6(I) = term6(I) * Th_eps2(I)*grad_s_avg(I)*8d0 + term6(I) * Th_eps2g(I) * 8d0
	 term7(I) = term7(I) * eps2_Th2(I)*grad_s_avg(I)*8d0 + term7(I) * eps2_Th2g(I) * 8d0
	 
	 term8(I) = term8(I) * Th3(I)*grad_s_avg(I)*8d0 + term8(I) * Th3g(I) * 8d0
	 term9(I) = term9(I) * eps_Th3(I)*grad_s_avg(I)*8d0 + term9(I) * eps_Th3g(I) * 8d0
	 term10(I) = term10(I) * eps2_Th3(I)*grad_s_avg(I)*8d0 + term10(I) * eps2_Th3g(I) * 8d0
	 term11(I) = term11(I) * eps3(I)*grad_s_avg(I)*8d0 + term11(I) * eps3g(I) * 8d0
	 term12(I) = term12(I) * Th_eps3(I)*grad_s_avg(I)*8d0 + term12(I) * Th_eps3g(I) * 8d0
	 term13(I) = term13(I) * Th2_eps3(I)*grad_s_avg(I)*8d0 + term13(I) * Th2_eps3g(I) * 8d0
	 term14(I) = term14(I) * Th3_eps3(I)*grad_s_avg(I)*8d0 + term14(I) * Th3_eps3g(I) * 8d0
	 term15(I) = term15(I) * eps4(I)*grad_s_avg(I)*8d0 + term15(I) * eps4g(I) * 8d0
	 term16(I) = term16(I) * Th1_eps4(I)*grad_s_avg(I)*8d0 + term16(I) * Th1_eps4g(I) * 8d0
	 term17(I) = term17(I) * Th2_eps4(I)*grad_s_avg(I)*8d0 + term17(I) * Th2_eps4g(I) * 8d0
	 term18(I) = term18(I) * Th3_eps4(I)*grad_s_avg(I)*8d0 + term18(I) * Th3_eps4g(I) * 8d0
	 term19(I) = term19(I) * Th4_eps4(I)*grad_s_avg(I)*8d0 + term19(I) * Th4_eps4g(I) * 8d0
	 term20(I) = term20(I) * Th4(I)*grad_s_avg(I)*8d0 + term20(I) * Th4g(I) * 8d0
	 term21(I) = term21(I) * eps1_Th4(I)*grad_s_avg(I)*8d0 + term21(I) * eps1_Th4g(I) * 8d0
	 term22(I) = term22(I) * eps2_Th4(I)*grad_s_avg(I)*8d0 + term22(I) * eps2_Th4g(I) * 8d0
	 term23(I) = term23(I) * eps3_Th4(I)*grad_s_avg(I)*8d0 + term23(I) * eps3_Th4g(I) * 8d0
	 
	 term24(I) = term24(I) * eps5(I)*grad_s_avg(I)*8d0 + term24(I) * eps5g(I) * 8d0
	 term25(I) = term25(I) * Th1_eps5(I)*grad_s_avg(I)*8d0 + term25(I) * Th1_eps5g(I) * 8d0
	 term26(I) = term26(I) * Th2_eps5(I)*grad_s_avg(I)*8d0 + term26(I) * Th2_eps5g(I) * 8d0
	 term27(I) = term27(I) * Th3_eps5(I)*grad_s_avg(I)*8d0 + term27(I) * Th3_eps5g(I) * 8d0
	 term28(I) = term28(I) * Th4_eps5(I)*grad_s_avg(I)*8d0 + term28(I) * Th4_eps5g(I) * 8d0
	 term29(I) = term29(I) * Th5_eps5(I)*grad_s_avg(I)*8d0 + term29(I) * Th5_eps5g(I) * 8d0
	 term30(I) = term30(I) * Th5(I)*grad_s_avg(I)*8d0 + term30(I) * Th5g(I) * 8d0
	 term31(I) = term31(I) * eps1_Th5(I)*grad_s_avg(I)*8d0 + term31(I) * eps1_Th5g(I) * 8d0
	 term32(I) = term32(I) * eps2_Th5(I)*grad_s_avg(I)*8d0 + term32(I) * eps2_Th5g(I) * 8d0
	 term33(I) = term33(I) * eps3_Th5(I)*grad_s_avg(I)*8d0 + term33(I) * eps3_Th5g(I) * 8d0
	 term34(I) = term34(I) * eps4_Th5(I)*grad_s_avg(I)*8d0 + term34(I) * eps4_Th5g(I) * 8d0
	 
	 term35(I) = term35(I) * eps6(I)*grad_s_avg(I)*8d0 + term35(I) * eps6g(I) * 8d0
	 term36(I) = term36(I) * Th1_eps6(I)*grad_s_avg(I)*8d0 + term36(I) * Th1_eps6g(I) * 8d0
	 term37(I) = term37(I) * Th2_eps6(I)*grad_s_avg(I)*8d0 + term37(I) * Th2_eps6g(I) * 8d0
	 term38(I) = term38(I) * Th3_eps6(I)*grad_s_avg(I)*8d0 + term38(I) * Th3_eps6g(I) * 8d0
	 term39(I) = term39(I) * Th4_eps6(I)*grad_s_avg(I)*8d0 + term39(I) * Th4_eps6g(I) * 8d0
	 term40(I) = term40(I) * Th5_eps6(I)*grad_s_avg(I)*8d0 + term40(I) * Th5_eps6g(I) * 8d0
	 term41(I) = term41(I) * Th6_eps6(I)*grad_s_avg(I)*8d0 + term41(I) * Th6_eps6g(I) * 8d0
	 term42(I) = term42(I) * Th6(I)*grad_s_avg(I)*8d0 + term42(I) * Th6g(I) * 8d0
	 term43(I) = term43(I) * eps1_Th6(I)*grad_s_avg(I)*8d0 + term43(I) * eps1_Th6g(I) * 8d0
	 term44(I) = term44(I) * eps2_Th6(I)*grad_s_avg(I)*8d0 + term44(I) * eps2_Th6g(I) * 8d0
	 term45(I) = term45(I) * eps3_Th6(I)*grad_s_avg(I)*8d0 + term45(I) * eps3_Th6g(I) * 8d0
	 term46(I) = term46(I) * eps4_Th6(I)*grad_s_avg(I)*8d0 + term46(I) * eps4_Th6g(I) * 8d0
	 term47(I) = term47(I) * eps5_Th6(I)*grad_s_avg(I)*8d0 + term47(I) * eps5_Th6g(I) * 8d0
	 
	 term48(I) = term48(I) * eps7(I)*grad_s_avg(I)*8d0 + term48(I) * eps7g(I) * 8d0
	 term49(I) = term49(I) * Th1_eps7(I)*grad_s_avg(I)*8d0 + term49(I) * Th1_eps7g(I) * 8d0
	 term50(I) = term50(I) * Th2_eps7(I)*grad_s_avg(I)*8d0 + term50(I) * Th2_eps7g(I) * 8d0
	 term51(I) = term51(I) * Th3_eps7(I)*grad_s_avg(I)*8d0 + term51(I) * Th3_eps7g(I) * 8d0
	 term52(I) = term52(I) * Th4_eps7(I)*grad_s_avg(I)*8d0 + term52(I) * Th4_eps7g(I) * 8d0
	 term53(I) = term53(I) * Th5_eps7(I)*grad_s_avg(I)*8d0 + term53(I) * Th5_eps7g(I) * 8d0
	 term54(I) = term54(I) * Th6_eps7(I)*grad_s_avg(I)*8d0 + term54(I) * Th6_eps7g(I) * 8d0
	 term55(I) = term55(I) * Th7_eps7(I)*grad_s_avg(I)*8d0 + term55(I) * Th7_eps7g(I) * 8d0
	 term56(I) = term56(I) * Th7(I)*grad_s_avg(I)*8d0 + term56(I) * Th7g(I) * 8d0
	 term57(I) = term57(I) * eps1_Th7(I)*grad_s_avg(I)*8d0 + term57(I) * eps1_Th7g(I) * 8d0
	 term58(I) = term58(I) * eps2_Th7(I)*grad_s_avg(I)*8d0 + term58(I) * eps2_Th7g(I) * 8d0
	 term59(I) = term59(I) * eps3_Th7(I)*grad_s_avg(I)*8d0 + term59(I) * eps3_Th7g(I) * 8d0
	 term60(I) = term60(I) * eps4_Th7(I)*grad_s_avg(I)*8d0 + term60(I) * eps4_Th7g(I) * 8d0
	 term61(I) = term61(I) * eps5_Th7(I)*grad_s_avg(I)*8d0 + term61(I) * eps5_Th7g(I) * 8d0
	 term62(I) = term62(I) * eps6_Th7(I)*grad_s_avg(I)*8d0 + term62(I) * eps6_Th7g(I) * 8d0
	 
	 term63(I) = term63(I) * Thg(I) * 8d0
	 term64(I) = term64(I) * epsg(I) * 8d0
	 
	 shearStress = D_p(IJK,M)*RO_s(M)*(term1(I)+ term2(I)+ term3(I)+term4(I)+term5(I)+term6(I)+term7(I)+ &
	                term8(I)+term9(I)+term10(I)+term11(I)+term12(I)+term13(I)+term14(I)+ &
			term15(I)+term16(I)+term17(I)+term18(I)+term19(I)+term20(I)+term21(I)+term22(I)+term23(I)+ &
			term24(I)+term25(I)+term26(I)+term27(I)+term28(I)+term29(I)+term30(I)+term31(I)+term32(I)+ &
			term33(I)+term34(I)+ &
...
!	write(*,'(I3, 8(1X,E13.6))') I, U_s(ijk,m), V_s(ijk,m), U_g(ijk), V_g(ijk), Ep_g(ijk), ep_s(ijk,m), Theta_m(ijk,m)	    
!	write(*,'(I3, 8(1X,E13.6))') I, Mu_s_v(IJK), x1*UxUy(I), shearStress/(D_p(IJK,M)*RO_s(M))
	Mu_s_v(IJK) = shearStress/(grad_s_avg(I)*8d0) - RO_S(M)*(x1*UxUy(I)+epsUxUy(I))/(grad_s_avg(I)*8d0)
	    
	    
	    
            
            Kth=75d0*RO_s(M)*D_p(IJK,M)*DSQRT(Pi*Theta_m(IJK,M))/384d0
            
            !!!!!!!!!!!!!!! no need for Kth_star in Gidaspow model
!     
!     granular conductivity in Mth solids phase
            Kth_s(IJK,M) = Kth/(ETA*G_0(IJK,M,M))*(ONE+2.4d0*ETA*G_0(IJK,M,M)*EP_s(IJK,M))**2 &
	                 + 4d0*ETA*G_0(IJK,M,M)*D_p(IJK,M)*ROP_s(IJK,M)*EP_S(IJK,M)
!     
