...
!   a)  O3 -> 1.5 O2
      ...
      Rxna = (1. - EP_g(IJK)) * C(1) * (RO_g(IJK)*X_g(IJK,O3)/MW_g(O3))
!
!
...
!    (1) O3
      IF(X_g(IJK, O3) .GT. ZERO) THEN
        RoX_gc(IJK, O3) = RXNA  * MW_g(O3) / X_g(IJK, O3)
      ELSE
        RoX_gc(IJK, O3) = 1.0e-9
      ENDIF

!     (2) O2
      R_gp(IJK, O2) = RXNA * MW_g(O2) * 1.5
