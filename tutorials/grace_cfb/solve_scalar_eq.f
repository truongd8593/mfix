...
!
! Under-relaxation factors Ohmega for scalars source terms
!      
      OHMEGA_P = 0.5
      OHMEGA_C = 0.5
!
...
!
	         ...
	         IF (First_It) THEN
		  Scalar_p_O(IJK, N) = 0.D0
		  Scalar_c_O(IJK, N) = 0.D0

		 END IF
		   
                  APO = ROP_G(IJK)*VOL(IJK)*ODT 
                  S_P(IJK) = APO + (  ZMAX(SUM_R_G(IJK)) &
		                    + OHMEGA_P*Scalar_p(IJK, N) +	&
				     (ONE- OHMEGA_P)*Scalar_p_O(IJK, N)	&    
				      )*VOL(IJK) 
                  S_C(IJK) =   APO*ScalarO(IJK,N) &
		             + Scalar(IJK,N)*ZMAX((-SUM_R_G(IJK)))*VOL(IJK) &
			     + (OHMEGA_C*Scalar_c(IJK, N)+ (ONE - OHMEGA_C) &
			     *Scalar_c_O(IJK, N)) *VOL(IJK)
		  
		  Scalar_p_O(IJK, N) = Scalar_p(IJK, N) 
		  Scalar_c_O(IJK, N) = Scalar_c(IJK, N)   
               ELSE 
!
                  S_P(IJK) = ZERO 
                  S_C(IJK) = ZERO 
