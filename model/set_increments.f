...
!
               ...
               IF (ICLASS > MAX_CLASS) THEN 
                  IF(DMP_LOG)WRITE (UNIT_LOG, 1000) MAX_CLASS 
                  CALL MFIX_EXIT(myPE) 
               ENDIF 
...
!                      Index for the solids phase.
      ...
      CHARACTER(LEN=64) :: IJK_FILE_NAME
...
! Step 1: Put all send and receive layer cells in a contiguous block, needed for parallel run only
      ...
      IF(MyPE == PE_IO.AND.NumPEs>1) WRITE(*,*)' Re-indexing: Arranging all send and receive layer cells, needed for parallel run only ...'
...
! Try to avoid pointing to a cell out of bound
      ...
      CALL ALLGATHER_1i (NCPP_UNIFORM(MyPE),NCPP_UNIFORM,IERR)
...
!       WRITE(*,*) 'set increment:',MyPE,NCPP_UNIFORM(MyPE),BACKGROUND_IJKEND3,IJKEND3
      ...
      CALL SHIFT_DP_ARRAY(SCALAR_NODE_XYZ,1)
      CALL SHIFT_DP_ARRAY(SCALAR_NODE_XYZ,2)
      CALL SHIFT_DP_ARRAY(SCALAR_NODE_XYZ,3)
