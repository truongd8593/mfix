...
!$omp              private( IJK, I, IJKE, EPGA, J, JM, K,  &
!$omp              IMJK,IJKN,IJKS,IJMK,IJKT,IJKTE,IJKNT,  &
!$omp              IJKM, IJMKP, IJKTW, IMJKP, IJKP, IM, IJKW, IJKST, KP, &
!$omp              SBV,  SSX,SSY, SSZ, &
!$omp              BCV,NOC_UG, uw_g, vw_g, ww_g, cut_tau_ug, bct,mu_gt_cut,del_h,nx,ny,nz,xi, vi,yi,zi,sx,sy,sz,ssy_cut,noc_wg,cut_tau_wg,ui,dudz_at_e,dudz_at_w,ssx_cut,u_node_at_wb,u_node_at_wt,u_node_at_eb,u_node_at_et,vxz,duodz,dvdz_at_S,dvdz_at_N,v_node_at_sb,v_node_at_st,v_node_at_nb,v_node_at_nt) &
...
!           SSX:
                  ...
                  IF(U_NODE_AT_ET.AND.U_NODE_AT_EB) THEN
                     ...
                     IF(NOC_WG) dudz_at_E = dudz_at_E - ((Ui-UW_g) * ONEoDZ_T_U(IJK)/DEL_H*(Sx*Nx+Sy*Ny))
                  ...
                  IF(U_NODE_AT_WT.AND.U_NODE_AT_WB) THEN
                     ...
                     IF(NOC_WG) dudz_at_W = dudz_at_W - ((Ui-UW_g) * ONEoDZ_T_U(IMJK)/DEL_H*(Sx*Nx+Sy*Ny))
                  ...
                  IF(U_NODE_AT_EB) THEN
                     ...
                     SSX_CUT = - MU_GT_CUT * (U_G(IJK) - UW_g) / DEL_H * (Nz*Nx) * Area_W_CUT(IJK)
                  ELSE
                     SSX_CUT =  ZERO
                  ENDIF
...
!           SSY:
                  ...
                  IF(V_NODE_AT_NT.AND.V_NODE_AT_NB) THEN
                     ...
                     IF(NOC_WG) dvdz_at_N = dvdz_at_N - ((Vi-VW_g) * ONEoDZ_T_V(IJK)/DEL_H*(Sx*Nx+Sy*Ny))
                  ...
                  IF(V_NODE_AT_ST.AND.V_NODE_AT_SB) THEN
                     ...
                     IF(NOC_WG) dvdz_at_S = dvdz_at_S - ((Vi-VW_g) * ONEoDZ_T_V(IJMK)/DEL_H*(Sx*Nx+Sy*Ny))
                  ...
                  IF(V_NODE_AT_NB) THEN
                     ...
                     SSY_CUT = - MU_GT_CUT * (V_G(IJK) - VW_g) / DEL_H * (Nz*Ny) * Area_W_CUT(IJK)
                  ELSE
                     SSY_CUT =  ZERO
                  ENDIF
...
!           SSZ:
                  ...
                  SSZ = MU_GT(IJKT)*(W_G(IJKP)-W_G(IJK))*ONEoDZ_T_W(IJK)*AXY_W(IJK) - &
                      ...
                      - MU_GT_CUT * (W_g(IJK) - WW_g) / DEL_H * (Nz**2) * Area_W_CUT(IJK)
