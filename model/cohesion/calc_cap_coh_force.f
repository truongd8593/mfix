...
!                                                                      C
!  Module name: CALC_CAP_COH_FORCE(I,J)                                C
!  Purpose: ! This module will calculate the force imposed on two      C
!           particles due to a capture-cohesive interaction            C
!                                                                      C
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      ...
      SUBROUTINE CALC_CAP_COH_FORCE(I,J)
...
!-----Convert momentum impulse into force on particle
      ...
      IF(COHESION_DEBUG.gt.0)THEN
         PRINT *,'**END CAPTURE CALC'
      END IF

      END SUBROUTINE CALC_CAP_COH_FORCE
