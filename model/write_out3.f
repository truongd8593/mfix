...
!  Purpose: To write cpu and wall time used by the code                C
!                                                                      C
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
!
      SUBROUTINE WRITE_OUT3(CPU,walltime)
...
!
      if (myPE.eq.PE_IO) then   !//d
         ...
         WRITE (*, *) ' Total wall time used = ', walltime, 'seconds'
         WRITE (*, *) ' '
         WRITE (*, 1000)
!C
         WRITE (UNIT_OUT, 1000)
         ...
         WRITE (UNIT_OUT, *) ' Total wall time used = ', walltime, 'seconds'
         WRITE (UNIT_OUT, *) ' '
         WRITE (UNIT_OUT, 1000)
...
!
      ...
      IF(DMP_LOG)WRITE (UNIT_LOG, *) ' Total wall time used = ', walltime, 'seconds'
      IF(DMP_LOG)WRITE (UNIT_LOG, *) ' '
      IF(DMP_LOG)WRITE (UNIT_LOG, 1000)
      CALL END_LOG
!
