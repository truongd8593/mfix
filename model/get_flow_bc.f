...
! Find the cell index value for the boundary cell in the direction
...
! yz-plane based on J_s and K_b.  Then ensure that it is true for J_s
! and K_b - K_t
...
! determine/set flow bc_plane
                  CALL MOD_BC_I (BCV, BC_I_W(BCV), BC_I_E(BCV), &
                     BC_J_S(BCV), BC_K_B(BCV), BC_PLANE(BCV)) 
                  I_WALL = BC_I_W(BCV)
! define fluid index relative to wall index                  
                  IF (BC_PLANE(BCV) == 'W') THEN 
! fluid cell is on west side of flow plane
                     I_FLUID = I_WALL - 1   
                  ELSE
! fluid cell is on east side of flow plane
                     I_FLUID = I_WALL + 1   
                  ENDIF 
...
! wall cell
                        IF (.NOT.(WALL_ICBC_FLAG(IJK_WALL) .AND. &
                           ...
                           IF(DMP_LOG)WRITE (UNIT_LOG, 1100) BCV,&
                              I_WALL, I_FLUID, J, K, IJK_WALL, &
                              ICBC_FLAG(IJK_WALL), IJK_FLUID, &
                              ICBC_FLAG(IJK_FLUID) 
                           CALL MFIX_EXIT(myPE) 
                        ENDIF 
                     ENDDO 
...
! determine/set flow bc_plane
                  CALL MOD_BC_J (BCV, BC_I_W(BCV), BC_J_S(BCV), &
                     BC_J_N(BCV), BC_K_B(BCV), BC_PLANE(BCV)) 
                  J_WALL = BC_J_S(BCV) 
...
! wall cell
                        IF (.NOT.(WALL_ICBC_FLAG(IJK_WALL) .AND. &
                            ICBC_FLAG(IJK_FLUID)(1:1)=='.')) THEN 
                           IF(DMP_LOG)WRITE (UNIT_LOG, 1200) BCV, &
                              I, J_WALL, J_FLUID, K, &
...
! determine/set flow bc_plane
                  CALL MOD_BC_K (BCV, BC_I_W(BCV), BC_J_S(BCV), &
                     BC_K_B(BCV), BC_K_T(BCV), BC_PLANE(BCV)) 
                  K_WALL = BC_K_B(BCV) 
...
! wall cell
                        IF (.NOT.(WALL_ICBC_FLAG(IJK_WALL) .AND. &
                            ICBC_FLAG(IJK_FLUID)(1:1)=='.')) THEN 
                           IF(DMP_LOG)WRITE (UNIT_LOG, 1300) BCV, I, &
                              J, K_WALL, K_FLUID, &
                              IJK_WALL, ICBC_FLAG(IJK_WALL), IJK_FLUID, &
                              ICBC_FLAG(IJK_FLUID) 
                           CALL MFIX_EXIT(myPE) 
