...
!-----------------------------------------------
      ...
      CALL READ_RES_DES(lNEXT_REC, PN_WALL(1,:))
      ...
      DO LC1=1, 6
         CALL READ_RES_DES(lNEXT_REC, PN_WALL(LC1,:))
         CALL READ_RES_DES(lNEXT_REC, PV_WALL(LC1,:))

         DO LC2=1, lDIMN
            CALL READ_RES_DES(lNEXT_REC, PFT(:,LC1,LC2))
            CALL READ_RES_DES(lNEXT_REC, PFT_WALL(:,LC1,LC2))
         ENDDO
      ENDDO

      CALL READ_RES_DES(lNEXT_REC, COLLISION_NUM)
      DO LC1=1, COLLISION_NUM
         CALL READ_RES_DES(lNEXT_REC, PV_COLL(LC1))
         DO LC2=1, lDIMN
            CALL READ_RES_DES(lNEXT_REC, PFN_COLL(LC2,LC1))
            CALL READ_RES_DES(lNEXT_REC, PFT_COLL(LC2,LC1))
         ENDDO
      ENDDO
