...
!direct copy in case of single processor
      ...
      if (ineighproc(pface).eq.mype) then
         drecvbuf(lrecvface)%facebuf(1:isendcnt(pface))= dsendbuf(pface)%facebuf(1:isendcnt(pface))
      else
         ltag = message_tag(ineighproc(pface),mype,pface)
         call des_mpi_irecv(drecvbuf(pface)%facebuf(:),imaxbuf, &
         ...
         call des_mpi_isend(dsendbuf(pface)%facebuf(:),isendcnt(pface), &
