...
! logic flag telling whether contact pair is old      
      LOGICAL ALREADY_NEIGHBOURS, consider_bc, consider_bc_temp
...
! local values used spring constants and damping coefficients
      ...
      INTEGER :: COUNT_BC, IJK_WALL, COUNT
...
! particle to cut-cell face)
      DOUBLE PRECISION  NORM1, NORM2, NORM3, NORMAL_MAG, WALL_COOR(DIMN)
      DOUBLE PRECISION X1MINX0(3), X2MINX1(3), TEMP_CROSSP(3), X1(3)
      
      integer RESTRICT_COUNT, RESTRICT_ARR(10), COUNT_REST
      ...
      DO LL = 1, MAX_PIP
         ...
         IF(DEBUG_DES.AND.LL.EQ.FOCUS_PARTICLE) THEN 
            IF (.NOT.DES_LOC_DEBUG) THEN
               ...
               WRITE(*,1000)
            ENDIF
            !WRITE(*,'(5X,A,I10)') 'Focus On Particle ', LL
            !WRITE(*,'(5X,A,2(ES15.7))') &
            ...
            WRITE(*,*) 'NUMBER OF BC = ', COUNT_BC, I_OF(IJK), J_OF(IJK), K_OF(IJK)
            
            WRITE(*,'(A, 3(2x, g17.8))') 'POS = ', DES_POS_NEW(LL, :)
            IF(COUNT_BC.GE.1) then 
               DO COUNT = 1, COUNT_BC 
                  WRITE(*,*) 'BC TYPE', COUNT, DES_CELLWISE_BCDATA(IJK)%BDRY_LIST(COUNT)%IJK_SCAL, DES_CELLWISE_BCDATA(IJK)%BDRY_LIST(COUNT)%DES_BC_TYPE
                  IF(TRIM( DES_CELLWISE_BCDATA(IJK)%BDRY_LIST(COUNT)%DES_BC_TYPE).eq.'NORMAL_WALL') write(*,'(A, 3(2x, g17.8))') 'NORMAL = ', DES_CELLWISE_BCDATA(IJK)%BDRY_LIST(COUNT)%NORMAL(:)
               ENDDO
               read(*,*)
            endif
         ENDIF
...
! a new 'entering' particle or is already marked as a potential exiting particle
         IF( .NOT.PEA(LL,2) .AND. .NOT.PEA(LL,3)) THEN
            ...
            IF(DEBUG_DES.AND.LL.EQ.FOCUS_PARTICLE) THEN 
               WRITE(*,*) 'going to check the bc for LL = ', LL
               WRITE(*,*) 'IJK. COUNT_BC, RLM, DISTMOD = ', IJK, COUNT_BC, R_LM, DISTMOD
            ENDIF
...
!                  WRITE(*,'(A35, 3(2x,g17.8))') 'X1MINX0 = ', X1MINX0(:)
!                  WRITE(*,'(A35, 3(2x,g17.8))') 'CROSS PRODUCT = ', TEMP_CROSSP(1:3)
!                  WRITE(*,'(A35, 3(2x,g17.8))') 'CROSS PRODUCT = ', TEMP_CROSSP(1:3)
...
!                  WRITE(*,*) 'DETECTED PARTICLE-WALL OVERLAP, IW = ', IW
                  ...
                  IF(IW.eq.5) THen 
                     ...
                     IF(CUT_CELL_AT(IJK)) THEN
                        ...
                        RESTRICT_ARR(RESTRICT_COUNT )  = IJK_WALL !for the cut-cell, ijk and ijk_wall shud be same
                     ENDIF
                  ENDIF
...
! collision
                  ...
                  IF(DEBUG_DES.AND.LL.EQ.FOCUS_PARTICLE) THEN
                     ...
                     WRITE(*,*) '     WALL COUNT =         =', WALL_COUNT
                     WRITE(*,*) '     WALL_TYPE =          =', WALL_TYPE
                     
                     WRITE(*,*) '     IJK_WALL =          =', IJK_WALL, I_OF(IJK_WALL), J_OF(IJK_WALL)
                     
                     WRITE(*,*) '     DISTMOD =             ', DISTMOD, DES_RADIUS(LL) - DISTMOD 
                     WRITE(*,*) '     OVERLAP_N, OVERLAP_T =', OVERLAP_N, OVERLAP_T
                     ...
                     WRITE(*,*) '     FT and FN= ', FTAN(:), FNORM(:)
                     WRITE(*,*) '     KT_W*OT*TAN = ', &
                     KT_DES_W*((OVERLAP_T)) *TANGENT(:)
                     WRITE(*,*) '     OVERLAP_T = ', OVERLAP_T, TANGENT
                     
                     FTMD = SQRT(DES_DOTPRDCT(FT_TMP,FT_TMP))
                     FNMD = SQRT(DES_DOTPRDCT(FNORM(1:DIMN),FNORM(1:DIMN)))
                     WRITE(*,*) '     FTMD, mu FNMD = ', FTMD, MEW_W*FNMD
                     
                     WRITE(*,*) ' FC = ', FC(LL,:)

                     IF(IW.EQ.5) THEN 
                        WRITE(*,*) 'CUT_FACE_LINE DETAILS'
                        WRITE(*,'(A35, 3(2x,g17.8))') 'LINE DIRECTION COSINE OR VEC(V)= ', X2MINX1(:)
                        WRITE(*,'(A35, 3(2x,g17.8))') 'LINE INTERCEPT OR VEC(CNOT) = ', X1(:)
