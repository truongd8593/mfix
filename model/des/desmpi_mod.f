...
! generic interface definition 
      interface des_gatherwrite 
         module procedure des_gatherwrite_l,des_gatherwrite_i,des_gatherwrite_d
      end interface 
...
! Purpose          : gathers double precision array from local to root 
!                    used for restart write
! Comments         : only particles belonging to the processor will be writen 
!                    variables related to gathering should be set before
!                    calling this subroutine   
! Parameters       : punit     - unit number 
!                    parray    - array to be writen
!                    ptotsize  - total array size 
...
!-----------------------------------------------
      ...
      if (mype .eq. pe_io) then 
         if (lconv2real) then 
            lrealarray = sngl(drootbuf)
            call out_bin_512r(punit,lrealarray,ptotsize,pnext_rec)
         else
            call out_bin_512(punit,drootbuf,ptotsize,pnext_rec)
         end if 
      end if 
...
! Purpose          : gathers integer array from local to root 
!                    used for writing restart files 
! Parameters       : punit - unit number 
!                    parray - source array (to be writen)
!                    ptotsize - total array size 
!                    pnext_rec - next rec pointer for file   
!------------------------------------------------------------------------
...
! Purpose          : writes active particle logical; used for dist_lO restart write  
! Parameters       : punit     - unit number 
!                    parray    - array to be writen
!                    ptotsize  - total array size 
!                    pnext_rec - next rec pointer for file   
...
!-----------------------------------------------
      ...
      do lcurpar = 1, max_pip 
         ...
         if (.not. pea(lcurpar,1)) cycle 
         if(parray(lcurpar)) then 
            lintarray(lparcount) = 1
         else
            lintarray(lparcount) = 0
         end if  
         lparcount = lparcount +1 
      end do  
...
!------------------------------------------------------------------------
! Subroutine       : des_writepar_i
! Purpose          : writes active particle integer; used for dist_IO restart write  
! Parameters       : punit     - unit number 
!                    parray    - array to be writen
!                    ptotsize  - total array size 
!                    pnext_rec - next rec pointer for file   
...
!-----------------------------------------------
      ...
      if(lloc2glb) then 
         do lcurpar = 1, max_pip 
            if (lparcount.gt.pip) exit
            if (.not. pea(lcurpar,1)) cycle 
            lintarray(lparcount) = iglobal_id(parray(lcurpar))
            lparcount = lparcount +1 
         end do  
      else 
         do lcurpar = 1, max_pip 
            if (lparcount.gt.pip) exit
            if (.not. pea(lcurpar,1)) cycle 
            lintarray(lparcount) = parray(lcurpar)
            lparcount = lparcount +1 
         end do  
      end if 
      call out_bin_512i(punit,lintarray,ptotsize,pnext_rec)
...
!------------------------------------------------------------------------
! Subroutine       : des_writepar_d 
! Purpose          : writes active particle; used for dist_IO restart write  
! Parameters       : punit     - unit number 
!                    parray    - array to be writen
!                    ptotsize  - total array size 
!                    pnext_rec - next rec pointer for file   
...
!-----------------------------------------------
      ...
      if(lconv2real) then 
         do lcurpar = 1, max_pip 
            if (lparcount.gt.pip) exit
            if (.not. pea(lcurpar,1)) cycle 
            lrealarray(lparcount) = real(parray(lcurpar))
            lparcount = lparcount +1 
         end do  
         call out_bin_512r(punit,lrealarray,ptotsize,pnext_rec)
      else 
         do lcurpar = 1, max_pip 
            if (lparcount.gt.pip) exit
            if (.not. pea(lcurpar,1)) cycle 
            ldblarray(lparcount) = parray(lcurpar)
            lparcount = lparcount +1 
         end do  
