...
!
      IF(GROUP_RESID) THEN
         ...
         WRITE (*, '(2X, 1PG7.1)',ADVANCE="NO") RESID_GRP(HYDRO_GRP)
         IF(GRANULAR_ENERGY) WRITE (*, '(2X, 1PG7.1)',ADVANCE="NO") RESID_GRP(THETA_GRP)
         IF(ENERGY_EQ) WRITE (*, '(2X, 1PG7.1)',ADVANCE="NO") RESID_GRP(ENERGY_GRP)
         IF(ANY_SPECIES_EQ) WRITE (*, '(2X, 1PG7.1)',ADVANCE="NO") RESID_GRP(SPECIES_GRP)
         IF(NScalar > 0) WRITE (*, '(2X, 1PG7.1)',ADVANCE="NO") RESID_GRP(SCALAR_GRP)
         IF(K_EPSILON) WRITE (*, '(2X, 1PG7.1)',ADVANCE="NO") RESID_GRP(KE_GRP)
...
!
         WRITE (*, '(I5)',ADVANCE="NO") NIT
         DO L = 1, 8
            IF (RESID_INDEX(L,1) /= UNDEFINED_I) WRITE (*, '(2X, 1PG7.1)',ADVANCE="NO") &
