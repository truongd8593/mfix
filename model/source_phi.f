...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvC
!                                                                      C
!  Module name: SOURCE_phi_Strong(S_p, S_c, EP, Phi, M, A_m, B_m, IER) 
!  Purpose: Determine source terms for phi eq. The terms               C
!  appear in the center coefficient and RHS vector.  The center        C
!  coefficient and source vector are negative.  S_p must be positive.  C
!                                                                      C
!                                                                      C
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
!
      SUBROUTINE SOURCE_PHI_Strong(S_P, S_C, EP, PHI, M, A_M, B_M, IER) 
...
!
!               Collect the terms
               A_M(IJK,0,M) = -S_P(IJK) + A_M(IJK,0,M)
               B_M(IJK,M) = -S_C(IJK)+B_M(IJK,M)
!			
	    ENDIF 
         ENDIF 
      END DO 
      RETURN  
      END SUBROUTINE SOURCE_PHI_Strong 
