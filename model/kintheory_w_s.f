...
!                                                                      
!  Module name: CALC_KTMOMSOURCE_W_S(KTMOM_W_s, IER)                     
!  Purpose: Determine source terms for W_S momentum equation arising   
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!
      SUBROUTINE CALC_KTMOMSOURCE_W_S(KTMOM_W_s, IER) 
!
!-----------------------------------------------
!     Modules
!-----------------------------------------------
...
!                      Sum total of momentum source terms
      DOUBLE PRECISION KTMOM_W_s(DIMENSION_3, DIMENSION_M) 
! 
!                      Phase index 
      INTEGER          M
! 
...
!-----------------------------------------------
      ...
      DO M = 1, MMAX 
          ...
          IF (TRIM(KT_TYPE) .EQ. 'IA_NONEP') THEN
               CALL CALC_IA_NONEP_W_S (KTMOM_W_s,M)
          ENDIF
...
!vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
!                                                                      
!  Module name: CALC_IA_NONEP_W_S(IANONEP_W_s, M)
!  Purpose: Determine source terms for W_S momentum equation arising   
!           from kinetic theory constitutive relations for stress
!           and solid-solid drag 
!
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!
      SUBROUTINE CALC_IA_NONEP_W_S(IANONEP_W_S, M) 
!
!-----------------------------------------------
!     Modules
!-----------------------------------------------
...
!-----------------------------------------------   
!                      Sum total of momentum source terms
      DOUBLE PRECISION IANONEP_W_s(DIMENSION_3, DIMENSION_M) 
!					
!                      Temporary variable
...
!                       
               ...
               IANONEP_W_S(IJK,M) = STRESS_TERMS + DRAG_TERMS
          ELSE
               IANONEP_W_S(IJK,M) = ZERO
          ENDIF     ! dilute
      ENDDO     ! over ijk
!
      RETURN  
      END SUBROUTINE CALC_IA_NONEP_W_S 
