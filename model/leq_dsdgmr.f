...
!                                                                      C
!  Module name: LEQ_DSDGMR(Vname, Var, A_m, B_m, M, ITMAX, IER)        C
!  Purpose: Diagonally scaled GMRES iterative sparse Ax=b solver: DSDGMR C
!                                                                      C
!                                                                      C
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
!
      SUBROUTINE LEQ_DSDGMR(VNAME, VAR, A_M, B_M, M, ITMAX, IER) 
...
!   Call SLAP routines
!
! 1.  Diagonally scaled GMRES iterative sparse Ax=b solver.
      CALL DSDGMR (N, B_M(1,M), VAR, NELT, IA, JA, A, ISYM, NSAVE, ITOL, TOL, &
         ITMAX, ITER, ERR, IERR, IUNIT, RWORK, LENW, IWORK, LENIW) 
!
!      IF(IERR .EQ. 1) THEN
!        LINE = "Error: Insufficient work space allocated for LEq Solver"
!        CALL WRITE_ERROR('LEQ_DSDGMR', LINE, 1)
!        STOP
!      ELSEIF(IERR .EQ. 2) THEN
!        LINE = "Warning: LEq Solver iteration has stalled"
!        CALL WRITE_ERROR('LEQ_DSDGMR', LINE, 1)
...
!        CALL WRITE_ERROR('LEQ_DSDGMR', LINE, 1)
!      ENDIF
!     Diagonally scaled GMRES iterative sparse Ax=b solver: DSDGMR
!------------------------------------------------------------------------------
      RETURN  
      END SUBROUTINE LEQ_DSDGMR 
