...
!    delta (2.mu/x.1/x.dw/dz)V |E-W                                    C
!                                                                      C
!  The following u-component CYLINDRICAL viscous stress tensor terms   C
!  are not calculated here as they                                     C
!  are handled elsewhere in the indicated routine:                     C
!  > part of the term -tau_zz/x xdxdydz =>                             C
!                     -2.mu/x. u/x xdxdydz                             C
!    delta(-2.mu.u/xp^2)V |p                                           C
!    calculation in source_u_g                                         C
!                                                                      C
!  To reconstitute the complete gas phase u-momentum viscous stress    C
...
!---------------------------------------------------------------------//
! TAU_U_g
      DOUBLE PRECISION, INTENT(OUT) :: TAU_U_g(DIMENSION_3)
...
!$omp              EPGA, EPMUGA, EPMU_GTE, EPMU_GBE,                   &
!$omp              SBV, SSX, SSY, SSZ, DWOXDZ, VTZB,                   &
!$omp              BCV, BCT, NOC_UG, uw_g, vw_g, ww_g,                 &
!$omp              del_h, nx, ny, nz, v_node_at_ne, v_node_at_sw,      &
!$omp              v_node_at_se, v_node_at_nw, w_node_at_bw,           &
!$omp              w_node_at_be, w_node_at_tw, w_node_at_te,           &
!$omp              xi, vi, yi, zi, sx, sy, sz, wi,                     &
!$omp              dvdx_at_S, dvdx_at_N, dwdx_at_B, dwdx_at_T,         &
!$omp              mu_gt_cut, cut_tau_ug, ssy_cut, ssz_cut)            &
!$omp           shared(ijkstart3, ijkend3, jm1, km1, ip1, k_of, i_of,  &
!$omp              j_of, do_k, cylindrical, cartesian_grid,            &
!$omp              cut_u_cell_at, bc_type, bc_u_id,                    &
!$omp              bc_uw_g, bc_vw_g, bc_ww_g, bc_hw_g,                 &
!$omp              ox_e, odx_e, ox, odx, odz, axy_u, axz_u, ayz,       &
!$omp              ayz_u, vol, vol_u, tau_u_g,                         &
!$omp              oneodx_e_u, oneodx_e_v, oneodx_e_w,                 &
!$omp              ep_g, mu_g, mu_gt, lambda_gt, trd_g, u_g, v_g, w_g, &
!$omp              x_u, y_u, z_u, x_v, y_v, z_v, x_w, y_w, z_w,        &
!$omp              wall_v_at, wall_w_at, area_u_cut,                   &
!$omp              cg_safe_mode, blocked_v_cell_at, blocked_w_cell_at)
...
!          1/x d/dx (lambda.trcD) xdxdydz
! delta (lambda.trcD)Ap |E-W : (i+1 - i-1), j, k
...
! delta (mu.dw/dx)Axy |T-B : at i+1/2, j, (k+1/2 - k-1/2)
               EPMU_GTE = AVG_X_H(AVG_Z_H(MU_GT(IJK),MU_GT(IJKT),K),&
                                  AVG_Z_H(MU_GT(IJKE),MU_GT(IJKTE),K),I)
               EPMU_GBE = AVG_X_H(AVG_Z_H(MU_GT(IJKB),MU_GT(IJK),KM),&
                                  AVG_Z_H(MU_GT(IJKBE),MU_GT(IJKE),KM),I)
               SSZ = EPMU_GTE*(W_G(IPJK)-W_G(IJK))*ODX_E(I)*&
                        AXY_U(IJK) - &
                     EPMU_GBE*(W_G(IPJKM)-W_G(IJKM))*ODX_E(I)*&
                        AXY_U(IJKM)
...
! shear stress terms
               IF(.NOT.CUT_U_CELL_AT(IJK))   THEN
                  ...
                  IF(DO_K) THEN
                     ...
                     EPMU_GBE = AVG_X_H(AVG_Z_H(MU_GT(IJKB),MU_GT(IJK),KM),&
                                        AVG_Z_H(MU_GT(IJKBE),MU_GT(IJKE),KM),I)
                     SSZ = EPMU_GTE*(W_G(IPJK)-W_G(IJK))*&
                              ONEoDX_E_W(IJK)*AXY_U(IJK) - &
                           EPMU_GBE*(W_G(IPJKM)-W_G(IJKM))*&
...
! SSZ:
                  IF(DO_K) THEN
                     ...
                     SSZ = EPMU_GTE*dwdx_at_T*AXY_U(IJK) - &
                           EPMU_GBE*dwdx_at_B*AXY_U(IJKM)  + SSZ_CUT
                  ELSE
                    SSZ = ZERO
                  ENDIF ! DO_K
...
! delta (mu(-w/x))Axy |T-B : at i+1/2, j, (k+1/2- k-1/2)
               SSZ = SSZ - ( &
                     EPMU_GTE*(HALF*(W_G(IPJK)+W_G(IJK))*OX_E(I))*&
                     AXY_U(IJK)-&
                     EPMU_GBE*(HALF*(W_G(IPJKM)+W_G(IJKM))*OX_E(I))*&
                     AXY_U(IJKM) )

! part of -tau_zz/x xdxdydz =>
!         -(2.mu/x).(1/x).dw/dz xdxdydz
! delta (2.mu/x.1/x.dw/dz)V |E-W : at i+1/2, j, k
